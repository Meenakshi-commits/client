...
const Signup = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      const response = await signup(values); // Call the signup API
          ...
          {({ isSubmitting }) => (
            <Form>
              ...
              <div className="mb-4">
                ...
                <Field type="email" name="email" className="w-full p-2 border rounded-lg" autoComplete="email" />
                <ErrorMessage name="email" component="div" className="text-red-500 text-sm" />
              </div>
              <div className="mb-4">
                <label className="block">Password</label>
                <Field type="password" name="password" className="w-full p-2 border rounded-lg" autoComplete="new-password" />
                <ErrorMessage name="password" component="div" className="text-red-500 text-sm" />
              </div>
              <div className="mb-4">
                <label className="block">Confirm Password</label>
                <Field type="password" name="confirmPassword" className="w-full p-2 border rounded-lg" autoComplete="new-password" />
